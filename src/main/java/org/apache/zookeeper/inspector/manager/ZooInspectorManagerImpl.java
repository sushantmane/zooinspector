...
//        System.out.println("[START] connected took: " + (System.currentTimeMillis() - start));
  ...
  Item getChildrenAndStat(String nodePath) throws KeeperException {
    ...
    try {
      ...
      LOG.error("Error occurred retrieving children of node: {}", nodePath, e);
    }
    return null;
  }
  ...
  public Map<String, String> getSessionMeta() {
    ...
    try {
      ...
      LOG.error("Error occurred retrieving session meta data.", e);
    }
    return sessionMeta;
  }
...
//    System.out.println("end");
  ...
  public void updateDefaultConnectionFile(Properties connectionProps) throws IOException {
    ...
    System.out.println("updateDefaultConnectionFile#connectString: " + sb);
    ...
    public void process(WatchedEvent event) {
      if (!closed) {
        try {
          ...
          LOG.error("Error occurred re-adding node watcher for node: {}", nodePath, e);
        }
